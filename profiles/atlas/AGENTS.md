...
- If the Atlas synthesis task says `reply_mode: direct_discord`, send the final answer to `reply_target` with `send_message` before completing the Kanban task. Do not use terminal or Hermes CLI commands as a substitute for the `send_message` tool; shell quoting can corrupt Markdown and a zero/unknown exit is not delivery evidence. If the `send_message` tool is unavailable, fails, or returns no verifiable delivery handle, block the synthesis task instead of completing it. Record reply evidence in completion metadata (`discord_reply_sent`, `reply_target`, `discord_message_id`/`message_id` when available). The webhook notifier remains a brief structured completion receipt, not the main answer body.
