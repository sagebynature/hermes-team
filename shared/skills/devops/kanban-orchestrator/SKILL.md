...
Use a dedicated board when the user names a project, organization, repo family, client, or long-lived workstream rather than a single task. Create the board before creating tasks so every child task lands in the right SQLite DB and dispatcher namespace.

```bash
hermes kanban boards create <slug> \
  --name "Human Name" \
  --description "Dedicated board for <scope>" \
  --icon "📌" \
  --color "#2563eb" \
  --switch
...
For GitHub organization / repo-family boards, seed one intake task before repo-specific work. See `references/github-org-board-intake.md` for a concrete session-derived example.

```bash
hermes kanban --board <slug> create \
  "Inventory <org/client> repositories and seed workstreams" \
  --assignee forge \
  --workspace scratch \
  --priority 80 \
  --idempotency-key <slug>-org-inventory-v1 \
  --body "Inventory visible repos; capture URL, language, description, updated date, purpose; flag active/stale/archived/duplicate/deployment-critical/docs-only repos; create child tasks for warranted follow-up; do not make code changes."
```

Pitfalls:
- Board slugs are immutable; use kebab-case (`nj-onnuri`), and put display formatting in `--name` (`NJ Onnuri`).
- Verify with `boards list` and `boards show` immediately after creation; creation can also switch the active board.
- Use `--board <slug>` on task commands even after `--switch` when replying from a gateway session, so the transcript proves which board was touched.
- Use an idempotency key for intake tasks so a repeated Discord request does not create duplicates.
