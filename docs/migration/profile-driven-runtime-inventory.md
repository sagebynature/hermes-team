...
| Per-agent homes | `agents/*/home/` | Legacy character/profile/skill seed state only. Runtime placeholders under legacy `logs/`, `memory/`, `sessions/`, `mcp/`, and `workspace/` trees were safe to remove because current generated Docker profile homes live under ignored `runtime/hermes/profiles/<profile>/`. Do not delete remaining legacy prompts, portraits, configs, or skills blindly; they may still contain useful seed material for future profile extraction. |
...
| Kanban dispatcher/notifier | `scripts/kanban-compose-dispatcher.py`, `scripts/kanban-mission-notifier.py`, Compose `kanban-*` services | Custom Compose dispatcher removed. Keep `scripts/kanban-mission-notifier.py` as the profile-compatible fan-in/outbox bridge; real native dispatch is hosted by `atlas-gateway`, while `kanban-dispatcher` is only an optional dry-run preview function service. |
